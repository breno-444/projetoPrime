...
class CadastroLogin {
  ...
  const CadastroLogin(this.titulo, this.icone);
}

const List<CadastroLogin> cadastrologins = <CadastroLogin>[
  CadastroLogin('Login', Icons.login),
  CadastroLogin('Cadastro', Icons.logout),
];
...
class Inicial extends StatefulWidget {
  @override
  InicialState createState() => InicialState();
}

class InicialState extends State<Inicial> {
  CadastroLogin cadastrologin = cadastrologins[0];

  void selecionar(CadastroLogin CadastroLoginEscolhido) =>
      setState(() => this.cadastrologin = CadastroLoginEscolhido);

  List<PopupMenuItem<CadastroLogin>> getListaItensMenu() {
    List<PopupMenuItem<CadastroLogin>> lista;

    lista = cadastrologins.skip(2).map(
      (CadastroLogin transporte) {
        return PopupMenuItem<CadastroLogin>(
          value: transporte,
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => InformacaoPage()),
              ),
            ),
            ListTile(
              title: Text('Feedback'),
